...
dtNASAMASExp2 = dtNASAMASExp2[!is_outlier(dtNASAMASExp2$MAS.alertness, coef = 1.5) & !is.na(dtNASAMASExp2$MAS.alertness), ]
...
by(summary_QuestExp2$median_MAS.alertness,summary_QuestExp2$Scenario, shapiro.test)
...
(MAS.alertnessmeansd = ggplot(summary_QuestExp2, aes(x = Timepoint, y = mean_MAS.alertness, group = Scenario, colour = Scenario))+ #summary_QuestExp2[!is_outlier(summary_QuestExp2$mean_MAS.alertness, coef= 1.5)]
  geom_line(aes(x = Timepoint, y = mean_MAS.alertness))+
  geom_ribbon(aes(ymin = mean_MAS.alertness - sd_MAS.alertness, ymax = mean_MAS.alertness + sd_MAS.alertness, fill = Scenario),alpha = 0.6, linewidth = 0.25)+
  scale_y_continuous(breaks = seq(-5,+5,1), limits = c(-5,+5), labels = c("-5\nVery tired\nsleepy","","","","", "0\nneutral","","","","", "+5\nVery alert,\nactive,\nenergetic")) +
  scale_x_discrete(labels = c("1","","","", "5","","","","", "10"))+
  labs(#title = Momentary Affect Scale - Energetic Arousal",
       x = "Time point",
       y = "MAS Energetic arousal",
       color = "mEDI [lx]",
       fill = "mEDI [lx]")+
...
print(MAS.alertnessmeansd)

my_comparisonsdtPVT = list(c("1 lx", "10 lx"),c("1 lx", "70 lx"),c("1 lx", "595 lx"),c("10 lx", "70 lx"),c("10 lx", "595 lx"), c("70 lx", "595 lx"))

set.seed(555)
(MAS.alertnessMedianAll=ggplot(summary_QuestExp2, aes(x = Scenario, y = median_MAS.alertness, group = Scenario, colour = Scenario, fill = Scenario)) + #[!is_outlier(summary_QuestExp2$median_MAS.alertness, coef= 1.5)]
  ...
  labs(#title = Momentary Affect Scale - Energetic Arousal",
       ...
       color = "mEDI [lx]",
       fill = "mEDI [lx]")+
  stat_pwc(hide.ns = TRUE, label = "p.adj.signif", y.position = 3, group.by = "x.var", method = "tukey_hsd", step.increase = 0.2, p.adjust.method = "bonferroni", parse = FALSE)+
  scale_y_continuous(breaks = seq(-5,+5,1), limits = c(-5,+5), labels = c("-5\nVery tired\nsleepy","","","","", "0\nneutral","","","","", "+5\nVery alert,\nactive,\nenergetic")) +
  ...
  theme(axis.text.x = element_text(size = 10),
        ...
        legend.title=element_text(size=10))+ 
  theme(legend.position = "none"))
print(MAS.alertnessMedianAll)


ggarrange(NULL,MAS.alertnessMedianAll, NULL, MAS.alertnessmeansd, hjust = -0.1, vjust =-0.5, nrow =4, common.legend = TRUE, legend = "bottom",labels = c("","A", "B"), heights = c(0.1,2.1,0.05, 1), font.label = list(size=10))
ggsave(filename = "Figure9-MASEnergetic.jpg",width = 10.5, height = 16.5, unit = "cm", dpi =350)
