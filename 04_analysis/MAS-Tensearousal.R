...
dtNASAMASExp2 = dtNASAMASExp2[!is_outlier(dtNASAMASExp2$MAS.comfort, coef = 1.5) & !is.na(dtNASAMASExp2$MAS.comfort), ]
...
(MAS.comfortmeansd = ggplot(summary_QuestExp2, aes(x = Timeframe, y = mean_MAS.comfort, group = Scenario, colour = Scenario))+ #summary_QuestExp2[!is_outlier(summary_QuestExp2$mean_MAS.comfort, coef= 1.5)]
  ...
  labs(#title = Momentary Affect Scale - Tense Arousal",
       x = "Time point",
       y = "MAS Tense Arousal",
       color = "Scenario [lx]",
       fill = "Scenario [lx]")+
...
set.seed(555)
(MAS.comfortMedianAll=ggplot(summary_QuestExp2, aes(x = Scenario, y = median_MAS.comfort, group = Scenario, colour = Scenario, fill = Scenario)) + #[!is_outlier(summary_QuestExp2$median_MAS.comfort, coef= 1.5)]
  ...
  theme(axis.text.x = element_text(size = 10),
        ...
        legend.title=element_text(size=10))+ 
  theme(legend.position = "none"))
print(MAS.comfortMedianAll)


ggarrange(NULL,MAS.comfortMedianAll, NULL, MAS.comfortmeansd, hjust = -0.1, vjust =-0.5, nrow =4, common.legend = TRUE, legend = "bottom",labels = c("","A", "B"), heights = c(0.1,2.1,0.05, 1), font.label = list(size=10))
ggsave(filename = "Figure8-MASTense.jpg",width = 10.5, height = 16.5, unit = "cm", dpi =350)
